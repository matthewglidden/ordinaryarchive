...
const VERBOSE_CATEGORY_LABEL_MAP = {
  "200+ HITS SEASON": CATEGORY_KEY_MAP.season_b_h_200,
  "200+ HITS SEASON BATTING": CATEGORY_KEY_MAP.season_b_h_200,
  ".300+ AVG SEASON": CATEGORY_KEY_MAP.season_b_avg_300,
  ".300+ AVG SEASON BATTING": CATEGORY_KEY_MAP.season_b_avg_300,
  "3000+ HITS CAREER": CATEGORY_KEY_MAP.career_b_h_3000,
  "3000+ HITS CAREER BATTING": CATEGORY_KEY_MAP.career_b_h_3000,
  "30+ HR SEASON": CATEGORY_KEY_MAP.season_b_hr_30,
  "30+ HR SEASON BATTING": CATEGORY_KEY_MAP.season_b_hr_30,
  "6+ WAR SEASON": CATEGORY_KEY_MAP.season_6_war,
  "BORN OUTSIDE US 50 STATES AND DC": CATEGORY_KEY_MAP.born_outside_usa,
  "HALL OF FAME": CATEGORY_KEY_MAP.hall_of_fame,
  "ONLY ONE TEAM": CATEGORY_KEY_MAP.career_single_team,
  "PLAYED FIRST BASE": CATEGORY_KEY_MAP.season_pos_1b,
  "PLAYED FIRST BASE MIN. 1 GAME": CATEGORY_KEY_MAP.season_pos_1b,
};

const CATEGORY_TITLE_MAP = VERBOSE_CATEGORY_LABEL_MAP;

const shortTeamLabel = (title, subtitle) => {
  const cleanTitle = title?.trim();
  const cleanSubtitle = subtitle?.trim();
  if (cleanSubtitle && TEAM_ABBREV_MAP[cleanSubtitle]) return cleanSubtitle;
  return cleanTitle ?? "";
};
...
const normalizeLabel = (value) => {
  ...
  const verbose = VERBOSE_CATEGORY_LABEL_MAP[cleaned.toUpperCase()];
...
export const canonicalGridLabel = (value) => displayLabelForValue(value);

const stableLabelFromValue = (value) => {
  const cleaned = cleanLabelText(value);
  if (!cleaned) return "";
  const canonical = canonicalGridLabel(cleaned);
  return canonical && canonical !== cleaned ? canonical : "";
};

const STABLE_LABEL_ATTRS = [
  "data-validator-key",
  "data-validatorkey",
  "data-category-key",
  "data-category-id",
...
const labelForElement = ($, element) => {
  const stableValues = STABLE_LABEL_ATTRS.map((attr) => $(element).attr(attr));
  const dataset = element.attribs || {};
  for (const [name, value] of Object.entries(dataset)) {
    if (/^data-/.test(name) && /validator|category|key/i.test(name)) {
      stableValues.push(value);
    }
  }

  for (const value of stableValues) {
    const stableLabel = stableLabelFromValue(value);
    if (stableLabel) return stableLabel;
  }

  return canonicalGridLabel($(element).attr("aria-label") || $(element).text());
};
...
const extractLabelValue = (value, nuxtArray) => {
  ...
  if (typeof resolved === "string") return canonicalGridLabel(resolved);
  ...
  for (const key of [
    ...
    if (typeof field === "string") {
      const stableLabel = stableLabelFromValue(field);
      if (stableLabel) return stableLabel;
    }
  }
  ...
  if (typeof resolvedTitle === "string") {
    ...
    if (teamLabel) return canonicalGridLabel(teamLabel);
    const categoryLabel = shortCategoryLabel(
      resolvedTitle,
      typeof resolvedShortTitle === "string" ? resolvedShortTitle : undefined
    );
    if (categoryLabel) return canonicalGridLabel(categoryLabel);
  }
  ...
  for (const key of [
    ...
    const field = resolveNuxtValue(record[key], nuxtArray);
    if (typeof field === "string") return canonicalGridLabel(field);
  }
...
const extractGridCellAriaLabels = ($) => {
  ...
  for (let i = 1; i < 4; i += 1) {
    ...
    const value = labelForElement($, cell);
