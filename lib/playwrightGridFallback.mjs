...
import { canonicalGridLabel } from "./gridIngestion.js";
...
const canonicalFromStableValue = (value) => {
  const cleaned = cleanRawLabel(value);
  if (!cleaned) return "";
  const canonical = canonicalGridLabel(cleaned);
  return canonical && canonical !== cleaned ? canonical : "";
};

const canonicalLabelFromEntry = (entry) => {
  for (const value of entry.stable || []) {
    const canonical = canonicalFromStableValue(value);
    if (canonical) return canonical;
  }

  return canonicalGridLabel(entry.aria || entry.text || "");
};
...
const extractLabelsFromPage = async (page) => {
  const rows = await page
    ...
    .evaluateAll((elements, stableAttrs) =>
      elements.map((element) => {
        const stable = stableAttrs
          .map((attr) => element.getAttribute(attr))
          .filter(Boolean);
        for (const attr of element.getAttributeNames()) {
          if (/^data-/.test(attr) && /validator|category|key/i.test(attr)) {
            const value = element.getAttribute(attr);
            if (value) stable.push(value);
          }
        }
        return {
          stable,
  ...
  const normalizedRows = rows.map(canonicalLabelFromEntry).filter(Boolean);
  const normalizedCols = cols.map(canonicalLabelFromEntry).filter(Boolean);
  ...
  const cells = await page
    ...
    .first()
    .locator("tr")
    .evaluateAll((rowsEls, stableAttrs) => {
      const rowEls = rowsEls.map((row) => {
        ...
        return cells.map((cell) => {
          const stable = stableAttrs
            .map((attr) => cell.getAttribute(attr))
            .filter(Boolean);
  ...
  if (cells.length >= 4) {
    ...
    const columnLabels = headerRow
      .slice(1, 4)
      .map(canonicalLabelFromEntry)
      .filter(Boolean);
    const rowLabels = cells
      .slice(1, 4)
      .map((row) => canonicalLabelFromEntry(row[0] || {}))
      .filter(Boolean);
